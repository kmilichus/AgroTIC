...
@Repository
@Scope("singleton")
public class RangoElementoDAO implements IRangoElementoDAO{

	private static final Log log = LogFactory.getLog(RangoElementoDAO.class);
	...
	public Rangoelementos merge(Rangoelementos detachedInstance) {
		...
		try {
			Rangoelementos result = (Rangoelementos) sessionFactory.getCurrentSession().merge(detachedInstance);
	...
	public Rangoelementos findById(BigDecimal id) {
		...
		try {
			Rangoelementos instance = (Rangoelementos) sessionFactory.getCurrentSession()
					.get("co.edu.icesi.demo.modelo.Rangoelementos", id);
			if (instance == null) {
	...
	public List<Rangoelementos> findByExample(Rangoelementos instance) {
		...
		try {
			List<Rangoelementos> results = (List<Rangoelementos>) sessionFactory.getCurrentSession()
					.createCriteria("co.edu.icesi.modelo.Rangoelementos").add(create(instance)).list();
